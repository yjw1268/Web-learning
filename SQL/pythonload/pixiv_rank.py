...
class Pixiv(object):
    ...
    def beautifulsoup(self, get_number, target_url):
        ...
        for j in pic_dl:
            ...
            sql = "SELECT pixiv_id FROM pixiv_rank WHERE pixiv_id = %s" % (pic_id)
            cursor.execute(sql)
            ...
            # print(local_url)
            # print("<br>")
            sql = "INSERT INTO pixiv_rank(mode,temp_rank,raw_url, pixiv_id,original_url,local_url,title, love) VALUES ('%s','%s','%s', '%s','%s','%s','%s', '%s')" % \
                  (str(target_url),str(i),str(pic_dl_url), pic_id, str(adapt_url), str(local_url), name, 0)
            # 执行sql语句
            cursor.execute(sql)
            db.commit()
            i += 1
